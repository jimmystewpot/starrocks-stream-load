...
pub fn is_retryable_error<E: std::error::Error>(error: &E) -> bool {
    ...
    retryable_patterns.iter().any(|pattern| error_string.contains(pattern))
}
...
"#);
    ...
    match exponential_backoff(&backoff_config, operation).await {
        Ok(retry_result) => {
            println!("✓ Operation succeeded after {} attempt(s)", retry_result.total_attempts);
            println!("  Total retries: {}", retry_result.retries_count);
            println!("  Total duration: {}ms", retry_result.total_duration_ms);
            println!("  Average time per attempt: {}ms", 
...
"#, i + 10, i, (i + 10) * 5));
    ...
    println!("✓ Completed {} out of 3 concurrent operations", successful_ops);
    println!("  Total time: {}ms", total_time.as_millis());
    println!("  Average time per operation: {}ms", total_time.as_millis() / 3);

    println!("\n✅ Exponential backoff demonstration completed successfully!");
    println!("   This pattern should be implemented at the application layer for production resilience");
