#![allow(clippy::print_stdout, clippy::needless_raw_string_hashes, clippy::uninlined_format_args, clippy::doc_markdown, clippy::duration_suboptimal_units, unused_imports, clippy::must_use_candidate, clippy::missing_errors_doc, clippy::missing_panics_doc, clippy::similar_names, clippy::single_match_else, clippy::cast_precision_loss, clippy::ignored_unit_patterns)]
...
"#;
    ...
    demonstrate_format("CSV (Tab)", &tab_manager, "csv_tab_load", Bytes::from(tab_data)).await?;
    ...
    demonstrate_format("JSON Array", &json_manager, "json_load", Bytes::from(json_data)).await?;
...
"#;
    ...
    demonstrate_format("NDJSON", &ndjson_manager, "ndjson_load", Bytes::from(ndjson_data)).await?;
...
"#;
    ...
    demonstrate_format("CSV (Quoted)", &quoted_manager, "quoted_load", Bytes::from(quoted_data)).await?;
...
async fn demonstrate_format(
    ...
    match manager.send_single_batch(&label, data).await {
        ...
        Err(error) => {
            ...
            println!("  Note: This is expected if the format or column mapping isn't configured correctly");
        }
    }
