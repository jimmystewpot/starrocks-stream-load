    ...
    if error_lower.contains("timeout") || 
       ...
       error_lower.contains("unreachable") {
        return ClassifiedError {
            ...
            recovery_message: "Network error detected. Retry with exponential backoff recommended.".to_string(),
        };
    }
    ...
    if error_lower.contains("503") || 
       ...
       error_lower.contains("overload") {
        return ClassifiedError {
            ...
            original_error: error.to_string(),
            recovery_suggested: true,
            recovery_message: "Server error detected. Retry with increased backoff recommended.".to_string(),
        };
    }
    ...
    if error_lower.contains("schema") || 
       ...
       error_lower.contains("invalid data") {
        return ClassifiedError {
            ...
            recovery_message: "Data validation error. Please check data format and schema compatibility.".to_string(),
        };
    }
    ...
    if error_lower.contains("authentication") || 
       ...
       error_lower.contains("403") {
        return ClassifiedError {
            ...
            original_error: error.to_string(),
            recovery_suggested: false,
            recovery_message: "Authentication error. Please check credentials and permissions.".to_string(),
        };
    }
...
impl ErrorHandler {
    ...
    pub fn handle_error(&self, operation: String, error: String) -> ClassifiedError {
        ...
        tracing::error!("Error classified as {:?}: {}", classified.category, classified.original_error);
    ...
    pub fn get_error_summary(&self) -> ErrorSummary {
        ...
        for error in errors.iter() {
            *summary.by_category.entry(error.category.clone()).or_insert(0) += 1;
        }
        ...
        loop {
            ...
            match operation().await {
                ...
                Err(error) => {
                    ...
                    let classified = self.handler.handle_error(operation_name.clone(), error_string);
                    ...
                    if !classified.category.is_retryable() || retry_count >= self.handler.config.max_retries {
                        tracing::error!("Operation failed after {} attempts. Error is non-retryable or max retries exceeded.", attempt);
                    ...
                    tracing::warn!("Retry {}/{} in {}ms due to: {}", 
                                 retry_count, self.handler.config.max_retries, delay_ms, classified.original_error);
                    ...
                    backoff_delay = Duration::from_millis(
                        (backoff_delay.as_millis() as u64 * 2).min(self.handler.config.max_delay_ms)
                    );
                }
            }
        }
...
impl<E: std::error::Error> std::fmt::Display for RecoveryError<E> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "RecoveryError in operation '{}' (attempt {}): {} (category: {:?})",
...
impl DataValidator {
    pub fn validate_csv_data(data: &[u8]) -> Result<(), ValidationError> {
        let csv_str = std::str::from_utf8(data)
            .map_err(|e| ValidationError::InvalidUtf8(e.to_string()))?;
        ...
        for (i, line) in lines.iter().enumerate() {
            ...
            if line_columns != columns {
                return Err(ValidationError::InconsistentColumns(
                    format!("Line {} has {} columns, expected {}", i + 1, line_columns, columns)
                ));
            }
        }
...
"#;
    ...
    let valid_data = Bytes::from(r#"id,name,value
1,RecoveryUser,25
...
"#);
    ...
    match executor.execute_with_recovery("stream_load_with_recovery".to_string(), operation).await {
        Ok(response) => {
    ...
    let invalid_data = Bytes::from(r#"id,name,value
...
"#);
    ...
    match executor.execute_with_recovery("stream_load_invalid_data".to_string(), operation).await {
        Ok(_) => {
    ...
    if !error_summary.recent_errors.is_empty() {
        ...
        for (i, error) in error_summary.recent_errors.iter().enumerate().take(3) {
            ...
            println!("    {}: {:?} ({}ms ago)", i + 1, error.category, duration.as_millis());
