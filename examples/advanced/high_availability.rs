        ...
        tokio::spawn(async move {
            let mut interval = tokio::time::interval(config.health_check_interval);
                ...
                loop {
                    ...
                    println!("📊 Performing health check on {} nodes...", nodes_guard.len());
                    ...
                    for node in nodes_guard.iter_mut() {
                        ...
                        }.check_node_health(node).await;
                        
                        println!("  {} - Status: {}, Response: {}ms, Success rate: {:.1}%",
                                node.url, node.health, node.response_time_ms, node.success_rate());
...
impl HAStreamLoadExecutor {
    ...
    pub async fn send_with_failover(
        ...
        loop {
            ...
            match self.manager.send_single_batch(label, data.clone()).await {
                ...
                Err(error) => {
                    let duration = start.elapsed();
                    println!("✗ Node {} failed in {}ms: {}", node.url, duration.as_millis(), error);
                    
                    if attempts >= max_failover_attempts {
                        return Err(FailoverError::AllNodesFailed(
                            format!("Failed after {} attempts across {} nodes", attempts, healthy_nodes.len())
                        ));
                    }
    ...
    pub async fn health_status(&self) -> HealthStatus {
        ...
        let healthy_count = nodes.iter().filter(|n| n.health == NodeHealth::Healthy).count();
        let degraded_count = nodes.iter().filter(|n| n.health == NodeHealth::Degraded).count();
        let unavailable_count = nodes.iter().filter(|n| n.health == NodeHealth::Unavailable).count();
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("  Health check interval: {}s", ha_config.health_check_interval.as_secs());
    println!("  Max consecutive failures: {}", ha_config.max_consecutive_failures);
    ...
    let health_monitor = Arc::new(HealthMonitor::new(fe_nodes.clone(), ha_config, manager_ref.clone()));
    ...
    for node in &nodes {
        println!("  {} - Health: {}, Success rate: {:.1}%, Response: {}ms",
                node.url, node.health, node.success_rate(), node.response_time_ms);
    }
    ...
    let test_data = Bytes::from(r#"id,name,value
1,HAUser1,25
2,HAUser2,30
3,HAUser3,35
"#);
    ...
    println!("  Healthy: {} ({})", 
            ...
            if health_status.total_nodes > 0 {
                format!("{:.1}%", (health_status.healthy_count as f64 / health_status.total_nodes as f64) * 100.0)
            } else {
                "N/A".to_string()
            });
    ...
    if health_status.total_nodes > 0 {
        let availability = (health_status.healthy_count as f64 / health_status.total_nodes as f64) * 100.0;
...
"#, i + 10, i, (i + 10) * 2));
        ...
        let handle = tokio::spawn(async move {
            executor.send_with_failover(&label, data).await
        });
    ...
    println!("  Average time per operation: {}ms", total_time.as_millis() / 5);
