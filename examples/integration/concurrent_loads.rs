...
impl ConcurrentLoadExecutor {
    ...
    pub async fn execute_concurrent(&self, operations: Vec<LoadOperation>) -> ConcurrentResult {
        ...
        for operation in operations {
            ...
            let handle = tokio::spawn(async move {
                ...
                let result = match manager
                    .send_single_batch(&operation.label, operation.data)
                    .await
                {
                    Ok(response) => {
        ...
        let completed_operations: Vec<
            Result<
                Result<CompletedOperation, Box<dyn std::error::Error + Send>>,
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let executor = ConcurrentLoadExecutor::new(
        manager_ref.clone(),
        max_concurrent,
...
"#,
    ...
    println!(
        ...
        result.metrics.average_duration_ms
    );
    println!(
        "  Operations/second: {:.1}",
    ...
    println!(
        "Creating {} high-throughput operations...",
...
"#,
    ...
    println!(
        "  Min duration: {}ms",
        result.metrics.min_duration_ms.unwrap_or(0)
    );
    println!(
        "  Max duration: {}ms",
        result.metrics.max_duration_ms.unwrap_or(0)
    );
...
"#,
    ...
    println!(
        "Created {} operations with mixed valid/invalid data",
        operations.len()
    );
    let result = executor.execute_concurrent(operations).await;
...
"#,
    ...
    print!(
        "Executing {} operations with rate limit (2 RPS)...",
        operations.len()
    );
    let start = Instant::now();
    ...
    println!(
        "  Expected minimum time: ~{}ms (6 ops / 2 RPS = 3 sec)",
        (6.0 / 2.0 * 1000.0) as u128
    );
    println!("  Actual time: {}ms", duration.as_millis());
    println!(
        "  Actual throughput: {:.1} ops/sec",
        duration.as_secs_f64() / result.operations.len() as f64
    );
    println!("  Success rate: {:.1}%", result.metrics.success_rate());
    ...
    println!(
        ...
        current_metrics.successful_operations
    );
    println!(
        "  Total failed operations: {}",
        current_metrics.failed_operations
    );
    println!(
